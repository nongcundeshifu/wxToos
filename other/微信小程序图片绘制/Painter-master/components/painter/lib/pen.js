...
export default class Painter {
    ...
    /**
     * 根据 borderRadius 进行裁减  绘制圆角，是依据arc函数再根据其角度大小绘制四个角的弧度设置而成。
     */
    ...
    _drawAbsImage(view) {
        ...
        //     rHeight = Math.round((view.sWidth / width) * height);
        //     rWidth = view.sWidth;
		// } else if (view.sHeight < view.sWidth){
        //     rWidth = Math.round((view.sHeight / height) * width);
        //     rHeight = view.sHeight;
        // }
		...
		} else if (view.css && view.css.mode === 'aspectFill'){
    ...
    _fillAbsText(view) {
        ...
        for (let i = 0; i < lines; ++i) {
			...
			// 这里有一个bug，如果start + alreadyCount <= view.text.length为false，并且，measuredWith > width
			// 那么岂不是会绘制超出的文本？ 更改代码：
			...
			// 或者：
			// while ((start + alreadyCount > view.text.length)) {
			...
			// 注意，源代码的此处有个隐藏bug，因为在上面的while中的start + alreadyCount <= view.text.length如果为false，则不会删减text，那么因为这次的text会超出绘制框，导致下面的
			// start < view.text.length 的判断为false，导致无法添加 ...  ，从而文本绘制会超出。
